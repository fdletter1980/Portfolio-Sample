...
1. GCP compute & persisted disk which can be leveraged:
...
2. Open Google SDK for GCE compute and run zingg container:

Command: sudo docker run -it zingg/zingg:0.4.0 bash



3. Open another Google SDK for GCE compute for moving files into container:
...
4. Create pre-trained data set (Optional)

Currently a pre-trained data set exists called "training.csv".



5. Run --phase findTrainingData
